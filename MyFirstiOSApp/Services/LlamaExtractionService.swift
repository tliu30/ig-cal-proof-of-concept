...
enum LlamaExtractionService {

    /// Whether the GGUF model file is present on disk.
    /// Used by tests to conditionally include Llama in parameterized test runs.
...
private final class LlamaModelManager {
    ...
    private static let modelFilename = "Qwen2.5-1.5B-Instruct-Q4_K_M"
    private static let modelExtension = "gguf"

    /// Resolves the model file path with a three-level fallback:
    /// 1. `LLAMA_MODEL_PATH` environment variable (for CI / custom setups)
    /// 2. App bundle (for on-device builds where the Run Script phase copied it in)
    /// 3. `#filePath`-based source tree path (for development / tests via Xcode)
    static var modelFilePath: String {
        ...
        // 2. App bundle (set by the optional "Copy LLM Model" build phase)
        if let bundlePath = Bundle.main.path(
            forResource: modelFilename, ofType: modelExtension
        ) {
            return bundlePath
        }
